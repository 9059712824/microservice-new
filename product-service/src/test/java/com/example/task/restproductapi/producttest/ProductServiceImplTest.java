...
class ProductServiceImplTest {
    ...
    void testGetAllProductByType() {
        when(productRepository.findByType(type)).thenReturn(productList);

        List<Product> result = productService.getAllProductByType(type);

        assertNotNull(result);
        assertEquals(2, result.size());
        verify(productRepository, times(1)).findByType(type);
    }
    ...
    void testGetAllProductByType_NotFound() {
        when(productRepository.findByType(type)).thenReturn(new ArrayList<>());
